"""Jina Reader adapter for public URL-to-Markdown retrieval."""
...
class JinaReaderAdapter:
    ...
    def fetch(
        ...
        *,
        ...
        """Return Jina Reader's normalized Markdown with source provenance."""
        ...
        try:
            ...
            if not isinstance(data, dict):
                raise TypeError("missing data object")
            content = data.get("content")
            final_url = data.get("url")
            if not isinstance(content, str) or not isinstance(final_url, str):
                raise TypeError("missing string data.content or data.url")
            ...
            self._emit_failure(error, call_id, started_at, started_monotonic, task, trace_id, base_metrics)
            ...
            error = FetchError(
                ...
                retryable=retryable,
                context={"url": request.url, "provider": self.provider_name, "status_code": status},
            )
            self._emit_failure(
                error,
                call_id,
                ...
                task,
                trace_id,
                {**base_metrics, "http_status": status, "retryable": retryable},
            )
            raise error from exc
        except (TypeError, ValueError, httpx.HTTPError) as exc:
            retryable = isinstance(exc, httpx.HTTPError)
            error = FetchError(
                "Jina Reader request failed" if retryable else "Jina Reader returned an invalid response",
                retryable=retryable,
                context={"url": request.url, "provider": self.provider_name},
            )
            self._emit_failure(error, call_id, started_at, started_monotonic, task, trace_id, base_metrics)
        ...
        emit_tool_call(
            ...
            task=task,
            trace_id=trace_id,
            metrics={**base_metrics, "http_status": response.status_code, "byte_count": len(content_bytes), "final_url": final_url},
        )
        return resource
